...
The current scaffold owns no terminal model and no persistent WebView toolbar. It owns the project light/dark terminal palettes and touch arbitration because those are product policy, while Layer 2 owns neutral Android state and operations. Long press maps public buffer cells into xterm's public `select()` API; Layer 3 does not parse terminal text or own a second selection buffer. In the selection PopupWindow POC, Layer 3 requests a bounded native selection-action surface through Layer 2 and consumes only the resulting Copy/Paste/Select all events. Future special keys, modifier bars, user themes, font selection, search UI, progress presentation, userland, and workspace features also belong here and require separate owner decisions.
