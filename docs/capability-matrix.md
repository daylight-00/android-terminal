...
| Explicit clipboard actions | selection APIs and `paste()` | Layer 2 runtime + Layer 3 action surface | Connected | Bounded Android `ClipboardManager` read/write | Long press delegates selection to xterm; a WebView-local Copy/Paste/Select all toolbar calls only public Layer 2 and xterm APIs; movable handles remain future policy |
