...
| Explicit clipboard actions | xterm selection APIs, DOM selection, and `paste()` | Layer 2 runtime | Connected | Bounded Android `ClipboardManager` read/write plus a neutral floating `ActionMode` command surface remain available | Current isolation POC leaves all one-finger gestures, DOM row selection, handles, Copy UI, and overflow pan to WebView; native Paste is deliberately absent |
...
The current scaffold owns project palettes and pinch font resizing. The row-selection isolation POC switches the official xterm renderer from WebGL to DOM, marks the rendered rows as browser-selectable, and gives WebView complete ownership of one-finger tap, long-press, selection, and overflow pan. A narrow Layer 3 adapter only suppresses xterm's competing touch-generated mouse-selection handler without cancelling browser defaults. The helper-textarea Paste target and `scrollLines()` fallback are deliberately absent. Layer 3 does not own PTY transport, WebMessagePort, JNI, lifecycle recovery, clipboard transport, link validation, Android font-scale adaptation, or any xterm private object. Native selection remains a device-validated POC limited to rendered viewport DOM.
