...
| Focus, IME, hardware keyboard | xterm DOM input and System WebView | Layer 2 runtime | Connected with bounds | WebView/xterm remain DOM-input authority; Android caches IME visibility from Activity-root `WindowInsets` and provides an explicit show-only request used after a hidden-IME Layer 3 tap | Optional key UI and gesture policy; visible-IME scroll, pinch, and selection preserve focus |
