...
| Explicit clipboard actions | xterm selection APIs, DOM selection, and `paste()` | Layer 2 runtime | Connected | Bounded Android `ClipboardManager` read/write plus a neutral floating `ActionMode` command surface remain available | Current Layer 3 POC leaves long-press DOM selection, handles, and Copy UI to WebView; the public helper textarea and `paste()` provide the native Paste target |
...
The current scaffold owns project palettes, pinch font resizing, and the movement threshold for the public `scrollLines()` fallback. The native touch-selection POC switches the official xterm renderer from WebGL to DOM, marks the rendered rows as browser-selectable, and leaves stationary long presses to WebView so Android owns selection handles and the standard action menu. A narrow Layer 3 adapter uses only public xterm surfaces: it stops touch-generated selection mousedown propagation without cancelling the browser default, positions `terminal.textarea` at the cursor as the native Paste target, and routes a captured native paste event once through `terminal.paste()`. Layer 3 does not own PTY transport, WebMessagePort, JNI, lifecycle recovery, clipboard transport, link validation, Android font-scale adaptation, or any xterm private object. Native selection is intentionally limited to rendered viewport DOM and remains a device-validated POC.
