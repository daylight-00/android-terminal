...
| Explicit clipboard actions | selection APIs and `paste()` | Layer 2 runtime + Layer 3 action surface | Floating ActionMode POC | Bounded Android `ClipboardManager` read/write plus `TYPE_FLOATING` `ActionMode.Callback2`; the native surface emits bounded selection-action events | Long press keeps xterm as selection authority; Copy/Paste/Select all call only public Layer 2 and xterm APIs; Copy closes the native toolbar while preserving the selected range; movable handles remain future policy |
