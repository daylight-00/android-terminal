...
| Focus, IME, hardware keyboard | xterm DOM input and System WebView | Layer 2 runtime | Connected with bounds | WebView/xterm remain DOM-input authority; Android caches IME visibility from Activity-root `WindowInsets`; Layer 3 releases retained xterm focus on an observed visible-to-hidden transition and reasserts that release at the start of each hidden-IME gesture, while the explicit show-only request is used only after a completed short tap | Optional key UI and gesture policy; scroll, pinch, and selection never change focus at touch boundaries |
| Explicit clipboard actions | selection APIs and `paste()` | Layer 2 runtime + Layer 3 action surface | Selection PopupWindow POC | Bounded Android `ClipboardManager` read/write plus a non-focusable `PopupWindow` using explicit WebView-viewport-to-screen coordinate conversion; the native surface emits bounded selection-action events | Long press keeps xterm as selection authority; Copy/Paste/Select all call only public Layer 2 and xterm APIs; Copy closes the native toolbar while preserving the selected range; movable handles remain future policy |
