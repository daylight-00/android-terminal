...
| Focus, IME, hardware keyboard | xterm DOM input and System WebView | Layer 2 runtime | Connected with bounds | WebView/xterm remain DOM-input authority; Android caches IME visibility from Activity-root `WindowInsets`; Layer 3 releases retained xterm focus exactly on an observed visible-to-hidden IME transition and uses the explicit show-only request only after a completed hidden-IME tap | Optional key UI and gesture policy; scroll, pinch, and selection never change focus at touch boundaries |
