...
def verify(root: Path) -> list[str]:
    ...
    require("versionCode 27" in build, "versionCode must identify the native touch-selection POC release", failures)
    require("versionName '0.25.0'" in build, "versionName must identify the native touch-selection POC release", failures)
    ...
    require("touchActivationAuthority: 'webview-default-unconsumed-tap'" in customization_js, "native touch-selection POC must leave unclaimed taps to WebView/xterm", failures)
    ...
    require("layer2.useDomRenderer('native-touch-selection')" in customization_js, "Layer 3 must select xterm's official DOM renderer", failures)
    ...
    require("webview-native-dom-selection-poc" in customization_js and "webview-native-action-mode" in customization_js, "native selection authority declaration is missing", failures)
    ...
    require("user-select: text !important" in customization_css and "pointer-events: auto !important" in customization_css, "DOM rows must be native-selectable", failures)
    require("syncNativePasteTarget" in customization_js and "Math.max(cellWidth, 80)" in customization_js and "Math.max(cellHeight, 32)" in customization_js, "public xterm helper textarea must expose a bounded native paste target", failures)
    ...
    require("handleNativePaste" in customization_js and "layer2.terminal.paste(event.clipboardData.getData('text/plain'))" in customization_js, "native paste must route once through the public xterm paste API", failures)
