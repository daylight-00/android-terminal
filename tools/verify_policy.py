...
def verify(root: Path) -> list[str]:
    ...
    require("versionCode 36" in build, "versionCode must identify the selection PopupWindow POC release", failures)
    require("versionName '0.28.0'" in build, "versionName must identify the selection PopupWindow POC release", failures)
    ...
    require("terminal-selection-toolbar" not in html, "selection PopupWindow POC must not retain a WebView toolbar", failures)
    ...
    require("layer2.platform.showSelectionActions({x, y})" in customization_js and "layer2.platform.hideSelectionActions()" in customization_js and "layer2.onSelectionAction" in customization_js, "Layer 3 must use the Layer 2 native selection-action facade", failures)
    require("layer2-android-popupwindow-copy-paste-select-all" in customization_js, "Layer 3 must report selection PopupWindow authority", failures)
    require("PopupWindow(" in platform_adapter and "PopupWindow.INPUT_METHOD_NOT_NEEDED" in platform_adapter and "SOFT_INPUT_ADJUST_NOTHING" in platform_adapter and "getLocationOnScreen" in platform_adapter and "getWindowVisibleDisplayFrame" in platform_adapter and "showAtLocation" in platform_adapter, "Layer 2 selection PopupWindow implementation is incomplete", failures)
    require("selection-actions-show" in terminal_contract and "selection-actions-hide" in terminal_contract and "platform-event" in terminal_contract, "native selection-action contract is incomplete", failures)
    require("android-selection-popupwindow" in terminal_contract and "android-selection-popupwindow" in contract_js, "selection PopupWindow native capability must match", failures)
    ...
    require("tap-only-layer3-ime-focus-v1" in terminal_contract and "tap-only-layer3-ime-focus-v1" in contract_js, "tap-only Layer 3 IME capability must match", failures)
